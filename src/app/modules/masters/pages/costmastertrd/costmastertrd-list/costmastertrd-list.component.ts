...
@Component({
  selector: 'app-costmastertrd-list',
  templateUrl: './costmastertrd-list.component.html',
  styleUrls: ['./costmastertrd-list.component.css']
})
export class CostmastertrdListComponent implements OnInit {
  ...
  canSAPCostTrdRefresh: boolean = false;
  ...
  ngOnInit() {
    ...
    if (currentUser.role_code === 'Admin' || currentUser.role_code === 'FM' || currentUser.role_code === 'PE_TRD') {
      this.canSAPCostTrdRefresh = true;
    }
  ...
  downloadSample() {
    let url = environment.apiUrl.substring(0, environment.apiUrl.length - 7) + 'uploads/sample/' + SampleEnum.trdcostmaster;
    FileSaver.saveAs(url, SampleEnum.trdcostmaster);
  }

  public getCount() {
    let filtermodel = { searchValue: this.searchValue }
    this.costmastertrdService.getCostMasterCount(filtermodel).subscribe(
      response => {
        if (response) {
  ...
  public getData(pageSize) {
    ...
    this.costmastertrdService.getRMCostmasterList(filtermodel).subscribe(
      response => {
  ...
  public getLog() {
    this.router.navigateByUrl('masters/costmastertrd/logs');
  }
  ...
  public exportfiles(file: any) {
    let url = environment.apiUrl.substring(0, environment.apiUrl.length - 7) + 'uploads/costmastertrd/logs/' + file;
    FileSaver.saveAs(url, file);
  }

  private saveData(fileresponse: any) {
    this.costmastertrdService.importcostmaster(fileresponse).subscribe(
      response => {
        if (response) {
          // this.savedResult = response.result;
          // this.isShowDownload = true;
          // let url = environment.apiUrl.substring(0, environment.apiUrl.length - 7) + 'uploads/costmastertrd/logs/' + this.savedResult.status_file_name;
  ...
  public onExport() {
    ...
    this.costmastertrdService.initiateExportCostMaster(filtermodel).subscribe(
      response => {
        if (response.responsedata && response.responsedata.statusCode == 200) {
          ...
          this.router.navigateByUrl('masters/costmastertrd/downloads');
  ...
  onProductMasterDelete(event) {
    this.costmastertrdService.deletecostmaster(event.model.index).subscribe(
      response => {
        if (response) {
          this.getCount();
  ...
  OpenOmpManageAddEditCostMasterPopup(id: number){
    if (id) {
      this.costmastertrdService.getCostMasterData(id).subscribe(
        response => {
          if (response.data) {
  ...
  OpenAddEditCostMasterPopup(id: number) {
    if (id) {
      this.costmastertrdService.getCostMasterData(id).subscribe(
        response => {
          if (response.data) {
            ...
            this.costMasterForm.landingcost.setValue(response.data[0].landingcost);
            this.costMasterForm.copperindex.setValue(response.data[0].copperindex);
            this.costMasterForm.cubase.setValue(response.data[0].cubase);
            this.costMasterForm.transferprice.setValue(response.data[0].transferprice);
      ...
      this.costMasterForm.landingcost.setValue(null);
  ...
  onOmpManageCostSaveClick(){
    ...
    this.costmastertrdService.saveompmanagecostmaster(ompmanagecostmastersavemodel).subscribe(
      response => {
        this.getCount();
        this.modalService.dismissAll();
  ...
  onCostSaveClick() {
    ...
    if (this.costMasterForm.landingcost.value && this.costMasterForm.landingcost.value > 0) {
      isstdcostavailable = true;
    }

    if (this.costMasterForm.copperindex.value && this.costMasterForm.cubase.value &&
      this.costMasterForm.transferprice.value) {
    ...
    let costmasterheader = Config.masterfilesheaders.costmastertrd;
    ...
    costmastersavemodel.push({
      ...
      [costmasterheader[3]]: this.costMasterForm.landingcost.value ? this.costMasterForm.landingcost.value : 0,
      [costmasterheader[4]]: this.costMasterForm.copperindex.value,
      [costmasterheader[5]]: this.costMasterForm.cubase.value ? this.costMasterForm.cubase.value : 0,
      [costmasterheader[6]]: this.costMasterForm.transferprice.value ? this.costMasterForm.transferprice.value : 0,
      [costmasterheader[7]]: this.costMasterForm.moq.value ? this.costMasterForm.moq.value : 1,
      [costmasterheader[8]]: this.costMasterForm.validfrom.value,
      [costmasterheader[9]]: this.costMasterForm.validto.value
    });

    this.costmastertrdService.savecostmaster(costmastersavemodel).subscribe(
      response => {
        this.getCount();
        this.modalService.dismissAll();
  ...
  onSAPRefreshClick(){
    let filtermodel = {};
    this.costmastertrdService.getLappCostSet(filtermodel).subscribe(
      response => {
        if (response.responsedata && response.responsedata.statusCode == 200) {
          this.getCount();
