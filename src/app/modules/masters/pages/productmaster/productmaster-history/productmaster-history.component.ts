...
@Component({
  selector: 'app-productmaster-history',
  templateUrl: './productmaster-history.component.html',
  styleUrls: ['./productmaster-history.component.css']
})
export class ProductmasterHistoryComponent implements OnInit {
  ...
  downloadFile(file: any, isStatus: boolean) {
    let url;
    if (isStatus)
      url = environment.apiUrl.substring(0, environment.apiUrl.length - 7) + 'uploads/productmaster/logs/' + file;
    else
      url = environment.apiUrl.substring(0, environment.apiUrl.length - 7) + 'uploads/productmaster/' + file;
