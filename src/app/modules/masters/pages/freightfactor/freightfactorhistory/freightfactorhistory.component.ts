...
@Component({
  selector: 'app-freightfactorhistory',
  templateUrl: './freightfactorhistory.component.html',
  styleUrls: ['./freightfactorhistory.component.css']
})
export class FreightfactorhistoryComponent implements OnInit {
  ...
  public downloadFile(file: any, isStatus: boolean) {
    let url;
    if (isStatus)
      url = environment.apiUrl.substring(0, environment.apiUrl.length - 7) + 'uploads/freightmaster/logs/' + file;
    else
      url = environment.apiUrl.substring(0, environment.apiUrl.length - 7) + 'uploads/freightmaster/' + file;

    FileSaver.saveAs(url, file);
  }

  public cancelClicked() {
    this.router.navigateByUrl('masters/freightfactormaster/list');
  }
