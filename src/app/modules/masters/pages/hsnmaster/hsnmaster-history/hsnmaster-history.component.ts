...
@Component({
  selector: 'app-hsnmaster-history',
  templateUrl: './hsnmaster-history.component.html',
  styleUrls: ['./hsnmaster-history.component.css']
})
export class HsnmasterHistoryComponent implements OnInit {
  ...
  downloadFile(file: any, isStatus: boolean) {
    let url;
    if (isStatus)
      url = environment.apiUrl.substring(0, environment.apiUrl.length - 7) + 'uploads/hsnmaster/logs/' + file;
    else
      url = environment.apiUrl.substring(0, environment.apiUrl.length - 7) + 'uploads/hsnmaster/' + file;
