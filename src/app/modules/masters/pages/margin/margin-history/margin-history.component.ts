...
@Component({
  selector: 'app-margin-history',
  templateUrl: './margin-history.component.html',
  styleUrls: ['./margin-history.component.css']
})
export class MarginHistoryComponent implements OnInit {
  ...
  downloadFile(file: any, isStatus: boolean) {
    let url;
    if (isStatus)
      url = environment.apiUrl.substring(0, environment.apiUrl.length - 7) + 'uploads/marginmaster/logs/' + file;
    else
      url = environment.apiUrl.substring(0, environment.apiUrl.length - 7) + 'uploads/marginmaster/' + file;
  ...
  public cancelClicked() {
    this.router.navigateByUrl('masters/margin/list')
  }
}