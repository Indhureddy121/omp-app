...
@Component({
  selector: 'app-costmasterstd-history',
  templateUrl: './costmasterstd-history.component.html',
  styleUrls: ['./costmasterstd-history.component.css']
})
export class CostmasterstdHistoryComponent implements OnInit {
  ...
  public downloadFile(file: any, isStatus: boolean) {
    let url;
    if (isStatus)
      url = environment.apiUrl.substring(0, environment.apiUrl.length - 7) + 'uploads/costmasterstd/logs/' + file;
    else
      url = environment.apiUrl.substring(0, environment.apiUrl.length - 7) + 'uploads/costmasterstd/' + file;
