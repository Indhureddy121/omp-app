...
@Component({
  selector: 'app-costmasterstd-downloads',
  templateUrl: './costmasterstd-downloads.component.html',
  styleUrls: ['./costmasterstd-downloads.component.css']
})
export class CostmasterstdDownloadsComponent implements OnInit {
  ...
  searchValue: string = '';
  downloadList: any[] = [];
  type: number = DownloadTypeEnum.costmastermfg;
  serverPath: string = '';
  ...
  downloadFile(file: any) {
    let url;
    url = environment.apiUrl.substring(0, environment.apiUrl.length - 7) + 'uploads/downloads/costmasterstd/' + file;
  ...
  headerClick(event: any) {
    ...
    var table = document.getElementById('table-costmasterstd');
  ...
  public cancelClicked() {
    this.router.navigateByUrl('masters/costmasterstd/list')
  }
  ...
  public restartExportFile(fileObj: any) {
    ...
    this.costmasterstdService.initiateExportCostMaster(filtermodelexport).subscribe(
      response => {
  ...
  getDownloadFileLink(fileName: string) {
    return this.serverPath + 'uploads/downloads/costmasterstd/' + fileName;
  }
}
