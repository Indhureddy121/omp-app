...
@Component({
  selector: 'app-offerapprovedrecords',
  templateUrl: './offerapprovedrecords.component.html',
  styleUrls: ['./offerapprovedrecords.component.css']
})
export class OfferapprovedrecordsComponent implements OnInit {
  ...
  offerapprovedrecordsdataList: any[] = [];
  ...
  getData() {
    ...
    this.reportsService.getofferapprovedrecordsdata(filter).subscribe(
        response => {
          if (response.responsedata && response.responsedata.statusCode == 200) {
            this.offerapprovedrecordsdataList = response.responsedata.data;
          }
        }, error => {
    ...
    onExport() {
      if (this.offerapprovedrecordsdataList && this.offerapprovedrecordsdataList.length == 0) {
        this.notificationService.showError('There is no data to export.');
        return;
      }
  
      let fileName = 'Offer Approved Records - ' + this.createdDate;
  
      this.appService.exportAsExcelFile(this.offerapprovedrecordsdataList, fileName);
    }
}
